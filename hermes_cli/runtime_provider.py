...
def normalize_custom_provider_id(value: str) -> str:
    ...
    try:
        provider = _get_named_custom_provider(value)
    except Exception:
        provider = None
    if provider:
        value = str(
            provider.get("provider_key") or provider.get("name") or value
        )
...
def resolve_runtime_provider(
    *,
    ...
    allow_codex_app_server: Set False for auxiliary loops that must use the
    provider's configured Hermes transport even when main turns opted into
    Codex app-server.
    """
