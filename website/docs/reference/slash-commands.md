...
| `/codex-runtime [auto\|codex_app_server\|on\|off]` | Toggle the optional [Codex app-server runtime](../user-guide/features/codex-app-server-runtime) for OpenAI/Codex models and configured named custom providers. `auto` (default) uses Hermes' standard runtime; `codex_app_server` hands eligible turns to a `codex app-server` subprocess for native shell, apply_patch, Codex-managed auth/provider configuration, and migrated Codex plugins. Effective on next session. |
